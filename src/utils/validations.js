...
export default function(data) {
	let error = '';
	const {useremail,userpassword, firstName, lastName,  phoneNumber, emailMobile,confirmPassword} = data;
	...
	if (useremail!==undefined) {
		let emptyValidationText = checkEmpty(useremail, 'email');
		if (emptyValidationText!=='') {
			return emptyValidationText;
		} else {
	...
	if (userpassword!==undefined) {
		let emptyValidationText = checkEmpty(userpassword,'password');
		if (emptyValidationText!=='') {
			return emptyValidationText;
		} else {
			let minLengthValidation = checkMinLength(userpassword, 6, 'password');
			if (minLengthValidation !== '') {
